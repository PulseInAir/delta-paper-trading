...
def calculate_greeks(option_type, S, K, T, r, sigma):
    ...
    sigma: Volatility (decimal, e.g., 0.40)
...
class PortfolioManager:
    def __init__(self, db: DatabaseManager, client: DeltaClient):
        self.db = db
        self.client = client
        self.mode = os.getenv("TRADING_MODE", "paper").lower() # 'paper' or 'live'
    ...
    def update_valuation(self, tickers_dict, products_dict):
        """
        Runs on every tick (1s). Updates positions prices, PnL, margin, and portfolio-level Greeks.
        """
        if self.mode == "live":
    ...
    def _update_paper_valuation(self, tickers_dict, products_dict):
        ...
        for pos in positions:
            ...
            if side == "buy":
                ...
                margin_usd = 0.0 # Long options require no margin, only premium
            else:
                pnl_usd = (entry_price - mark_price) * contract_value * size
                # Short options require margin. Conservative estimate: 12% of underlying notional
    ...
    def _update_live_valuation(self, tickers_dict, products_dict):
        ...
        if pos_res.get("success"):
            ...
            # Clear old positions not in API
            ...
            # Save/Update active API positions
            for pos in api_positions:
                ...
                # Retrieve volatility and expiry
                ticker = tickers_dict.get(symbol)
                strike_price = float(prod.get("strike_price", 0.0))
    ...
    # Executing orders in Paper Mode
    def execute_paper_order(self, symbol, product_id, underlying, side, size, price, contract_value):
        """
        Executes a paper order, deducting premium/margin, recording a trade, and modifying position size.
        """
        fee_rate = 0.0005 # 0.05% execution fee
        ...
        if side == "buy":
            # Buying option: premium is deducted
            ...
            if pos:
                ...
                if pos["side"] == "buy":
                    ...
                    if pos["size"] > size:
                        ...
                        self.cash_inr += (pos["margin"] / pos["size"]) * size # release partial margin
                        # Remaining position keeps entry price
                        ...
                        self.cash_inr += pos["margin"] # release all margin
                        self.db.delete_position(symbol)
            ...
            # Selling/Shorting option: premium is added, but margin is blocked
            # Let's estimate spot price of underlying pair
            # For margin checking, we block 12% of underlying spot value
            # Since we don't have underlying spot, let's assume it's roughly near strike
            margin_usd = 0.12 * price * contract_value * size # default estimate
            margin_inr = margin_usd * FIXED_INR_USD_RATE
    ...
    def place_order(self, symbol, product_id, underlying, side, size, price, contract_value):
        if self.mode == "live":
            # Execute actual trade on Delta Exchange India
            # side is 'buy' or 'sell'
    ...
    def close_position(self, symbol):
        """
        Manually trigger closing an entire open position.
        """
        positions = self.db.load_positions()
        pos = next((p for p in positions if p["symbol"] == symbol), None)
        if not pos:
        ...
        price = pos["mark_price"] # close at current mark price (market order)
        
        # Load products specs for contract value
        # In a real situation, we can query details or default
